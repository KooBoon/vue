...
export default {
  ...
  methods: {
    order () {
      //发送请求，完成创建订单的操作
      this.$http.post('/order/create', { address: this.addressId, note: this.note, goods: this.buy }).then(res => {
        if (res.data.code === 0) {
          this.$toast(res.data.msg)
        } else if (res.data.code === 1) {
          //提示创建订单成功，跳转到商品订单列表页面
          this.$toast(res.data.msg)
    ...
    getGoodsList () {
      ...
      this.$http.get('shopcart', { params: { ids: idArr } }).then(res => {
        ...
        this.goodslist.forEach(item => {
          //通过getBuy得到商品id对应的购买数量
          item.count = this.getBuy[item.id].count
          amount += item.count * item.price
        })
        //将总价格保存到data中的anmount
        this.amount = amount
      })
    }
  }
